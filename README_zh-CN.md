...
点击额度池卡片中的 `5 HOURS / 1 WEEK` 按钮，可以切换驱动百分比、进度条、重置倒计时和恢复日期的额度窗口。监听器按 `windowDurationMins` 识别窗口，并在每次正常响应中同时缓存两个额度池；切换只改变本地视图，不会额外请求额度接口，也不会打断主动静默。选择会在皮肤或监听器重启后保留。如果账户只返回一个窗口，面板会用回退色明确显示实际可用窗口，而不会错误标注。
