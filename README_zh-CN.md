...
- 5 小时额度池在重置后仍保持选中，并在额度池卡片中提供持久化的 `5 HOURS / 1 WEEK` 切换按钮。
...
| `QUIET` | 不自动轮询；仍可手动刷新 | 完全静止，但继续监听任务状态 |
...
点击额度池卡片中的 `5 HOURS / 1 WEEK` 按钮，可以切换驱动百分比、进度条、重置倒计时和恢复日期的额度窗口。监听器按 `windowDurationMins` 识别窗口，点击后先显示目标池的缓存值，再主动查询一次最新额度。在 `QUIET` 状态下，这次查询完成后继续保持静默；任务活动时，下次自动查询安排在本次刷新完成的 20 秒后。选择会在皮肤或监听器重启后保留。如果账户只返回一个窗口，面板会用回退色明确显示实际可用窗口，而不会错误标注。
...
第一条命令会编译原生监听器、在 `dist` 中生成 `.rmskin`，并写入 `SHA256SUMS.txt`；第二条命令运行额度池回归测试。生成的可执行文件和安装包不会提交到源码仓库。
